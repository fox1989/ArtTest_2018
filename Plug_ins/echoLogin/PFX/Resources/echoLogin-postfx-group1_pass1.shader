...
#pragma multi_compile ECHO_PFX_CUSTOM_FRAG_1_OFF ECHO_PFX_CUSTOM_FRAG_1_ON
...
#ifdef ECHO_PFX_CUSTOM_FRAG_1_ON
fixed2  horz =  _echoScreen_TexelSize.xy * 0.5;
fixed3 _rgb1 = tex2D ( _echoScreen, v.tc1.xy + horz   ).xyz;
fixed3 _rgb2 = tex2D ( _echoScreen, v.tc1.xy - horz  ).xyz;
fixed3 _rgb3 = tex2D ( _echoScreen, v.tc1.xy + horz * fixed2(0,-1)  ).xyz;
fixed3 _rgb4 = tex2D ( _echoScreen, v.tc1.xy - horz * fixed2(0,-1) ).xyz;  
_ioRGB.xyz = ( _rgb1 + _rgb2 + _rgb3 + _rgb4 + _ioRGB.xyz ) / 5;
// custom blur code example
#endif
