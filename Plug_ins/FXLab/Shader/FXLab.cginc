...
fixed3 sampleColorBlurredFunc(sampler2D tex, float2 uv, float4 area, float2 texelSize, float radius)
{
	float2 size = area.zw - area.xy;
	uv = area.xy + uv * size;
...
float DistanceRayReflectionPlane(float3 rayPos, float3 rayDir, float4 planeEquation)
{
	float b = dot(rayDir, planeEquation.xyz);
	if (b <= 0)
		return MAX_FLOAT;
		
	float t = (dot(rayPos, planeEquation.xyz) + planeEquation.w) / b;
	return t;
}
