...
public class FXReflectionTexture : FXTexture
{
	...
	public override void Render(Camera renderCamera)
	{
        ...
        var oldPos = GetComponent<Camera>().transform.position;
        var newPos = reflectionMatrix.MultiplyPoint(oldPos);
        renderCamera.worldToCameraMatrix = GetComponent<Camera>().worldToCameraMatrix * reflectionMatrix;
        ...
        var euler = GetComponent<Camera>().transform.eulerAngles;
