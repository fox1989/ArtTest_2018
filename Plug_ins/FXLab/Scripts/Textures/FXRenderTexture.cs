...
public class FXRenderTexture : ScriptableObject
{
	...
	public Vector2 CalculateSize(Vector2 chartSize)
	{
        ...
        if (FXLab.UseNativeRenderTextures)
        {
            ...
            {
                size.x = Mathf.NextPowerOfTwo((int)size.x);
                size.y = Mathf.NextPowerOfTwo((int)size.y);
            }
	...
	public void SetShaderData(Material target, string name)
    {
        ...
        Texture texture;
        if (FXLab.UseNativeRenderTextures)
            texture = Registration.Chart.Data.RenderTexture;
        else
            texture = Registration.Chart.Data.Texture;
       
        Vector4 normalizedArea;
        Vector4 normalizedFastArea;

        if (FXLab.UseNativeRenderTextures)
        {
            normalizedArea = new Vector4(0, 0 + Registration.Chart.Data.TexelSize.y, 1, 1 + Registration.Chart.Data.TexelSize.y);
            normalizedFastArea = normalizedArea;
        }
        else
        {
            ...
            normalizedArea = new Vector4(
                 area.xMin / texture.width, area.yMin / texture.height,
                 area.xMax / texture.width, area.yMax / texture.height);
            normalizedFastArea = new Vector4(
                fastArea.xMin / texture.width, fastArea.yMin / texture.height,
                fastArea.xMax / texture.width, fastArea.yMax / texture.height);
        }

        SetShaderDataValues(target, name, texture, Registration.Chart.Data.TexelSize, normalizedArea, normalizedFastArea);
